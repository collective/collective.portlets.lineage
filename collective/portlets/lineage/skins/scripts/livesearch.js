...
var livesearch = (function () {
    ...
    function _searchfactory($form, $inputnode) {
        ...
        var $lastsearch = null,
            $request = null,
            $cache = {},
            $querytarget = $form.attr('action').replace(/search_portlet_results$/g,"") + "livesearch_reply",
            $$result = $form.find('div.LSResult'),
            $shadow = $form.find('div.LSShadow'),
            $path = $form.find('input[name=path]'),
            $subsite_paths = $form.find("input[name=subsite_paths]").val(),
            $parent_only = $form.find("input[name=parent_only]").val();
